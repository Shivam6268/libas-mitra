...
import { applyCouponOnCart } from '../feature/cart/cartSlice';
import { toast } from 'react-toastify'
import { placeOrder } from '../feature/order/orderSlice';
...
const OrderSummery = ({cart}) => {
    ...
    const handleApplyCoupon = () => {
        dispatch(applyCouponOnCart({couponCode}))
    }

    const checkToProceed = (orderDetails) => {
    ...
    let couponDiscount = coupon ? subTotal * coupon.couponDiscount / 100 : 0
            ...
            <div className="bg-white rounded-3xl shadow-md p-6 sticky top-6">
                ...
                <div className="mb-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        />
                        <button onClick={handleApplyCoupon}  className="px-6 py-3 bg-violet-600 text-white rounded-xl font-semibold hover:bg-violet-700 transition-colors">
                            Apply
                        </button>
                    </div>
                    {
                        coupon && <p className="text-xs text-green-500 mt-2">{coupon.couponCode} Applied : You Are Getting {coupon.couponDiscount}% Discount</p>
                    }
                </div>

                <div className="mb-6">
                    <label htmlFor="shippingAddress" className="block text-sm font-semibold text-neutral-900 mb-2">
                        Shipping Address
                    </label>
                ...
                <button onClick={() => checkToProceed({shippingAddress, coupon : coupon?.couponCode ? coupon?.couponCode : ""})} className="w-full bg-gradient-to-r from-violet-600 to-purple-600 text-white py-4 rounded-xl font-bold hover:from-violet-700 hover:to-purple-700 transition-all shadow-lg mb-3">
                    Proceed to Checkout
                </button>
